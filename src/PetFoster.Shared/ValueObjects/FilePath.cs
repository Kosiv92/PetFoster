...
public sealed record FilePath
{
    ...
    {
        return string.IsNullOrWhiteSpace(fullPath) ? (Result<FilePath, Error>)Errors.General.ValueIsInvalid("Path to file must not be empty") : (Result<FilePath, Error>)new FilePath(fullPath);
    }
}
