...
public sealed class Characteristics : ComparableValueObject
{
    ...
    public static Result<Characteristics, Error> Create(double weight, double height)
    {
        return weight <= 0
            ...
            : height <= 0
                ? (Result<Characteristics, Error>)Errors.General.ValueIsInvalid("The height must be greater than 0")
                : (Result<Characteristics, Error>)new Characteristics(weight, height);
    }
